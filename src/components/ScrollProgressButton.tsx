...
export const ScrollProgressButton = () => {
  ...
  return (
    <button
      ...
      className={cn(
        'fixed bottom-8 right-8 z-50 w-16 h-16 rounded-full shadow-lg border border-border/10 flex items-center justify-center transition-all duration-300 ease-in-out',
        'hover:scale-110 active:scale-95',
        isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8 pointer-events-none',
        {
          'bg-primary': isAtBottom, // Blue background when at bottom
          'bg-card/80 backdrop-blur-sm': !isAtBottom, // Default card background
        }
      )}
    >
      <div className="relative w-full h-full flex items-center justify-center">
        {/* Progress Ring SVG - Hidden when at bottom */}
        {!isAtBottom && (
          <svg
          ...
          >
            ...
            <circle
              ...
              fill="none"
            />
            {/* Foreground (Progress) Circle - Always Blue */}
            <circle
              cx="22" cy="22" r="20"
              strokeWidth="3"
